; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py UTC_ARGS: --version 5
; ModuleID = './riscv-esp32p4-load-store-1.c'
; Test ASM generation (Intrinsic -> ASM)
; RUN: llc -O2 -mattr=+xespv2p1,+espv-lowering -mtriple=riscv32 %s -o - | FileCheck %s --check-prefix=ASM

define dso_local void @test_ip(ptr noundef %src, ptr noundef %dst) local_unnamed_addr #0 {
; ASM-LABEL: test_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vld.128.ip q0, a0, 16
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    ret
entry:
  %vld1 = tail call { <16 x i8>, ptr } @llvm.riscv.esp.vld.128.ip(ptr %src, i32 16)
  %ev1 = extractvalue { <16 x i8>, ptr } %vld1, 0
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %ev1, ptr %dst, i32 16)
  ret void
}

define dso_local void @test_xp(ptr noundef %src, ptr noundef %dst, i32 noundef %incr) local_unnamed_addr #0 {
; ASM-LABEL: test_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vld.128.xp q0, a0, a2
; ASM-NEXT:    esp.vst.128.xp q0, a1, a2
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <16 x i8>, ptr } @llvm.riscv.esp.vld.128.xp(ptr %src, i32 %incr)
  %ev1 = extractvalue { <16 x i8>, ptr } %v1, 0
  %v2 = tail call ptr @llvm.riscv.esp.vst.128.xp(<16 x i8> %ev1, ptr %dst, i32 %incr)
  ret void
}

define dso_local ptr @test_ld_128_usar_ip(ptr noundef %src) local_unnamed_addr #1 {
; ASM-LABEL: test_ld_128_usar_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.ld.128.usar.ip q0, a0, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <16 x i8>, ptr, i32 } @llvm.riscv.esp.ld.128.usar.ip(ptr %src, i32 16)
  %ev1 = extractvalue { <16 x i8>, ptr, i32 } %v1, 1
  ret ptr %ev1
}

define dso_local ptr @test_ld_128_usar_xp(ptr noundef %src, i32 noundef %incr) local_unnamed_addr #1 {
; ASM-LABEL: test_ld_128_usar_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.ld.128.usar.xp q0, a0, a1
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <16 x i8>, ptr, i32 } @llvm.riscv.esp.ld.128.usar.xp(ptr %src, i32 %incr)
  %ev1 = extractvalue { <16 x i8>, ptr, i32 } %v1, 1
  ret ptr %ev1
}

define dso_local void @test_vld_h_64_ip(ptr noundef %src, ptr noundef %dst) local_unnamed_addr #0 {
; ASM-LABEL: test_vld_h_64_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vld.h.64.ip q0, a0, 8
; ASM-NEXT:    esp.vst.h.64.ip q0, a1, 8
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <8 x i8>, ptr } @llvm.riscv.esp.vld.h.64.ip(ptr %src, i32 8)
  %ev1 = extractvalue { <8 x i8>, ptr } %v1, 0
  %v2 = tail call ptr @llvm.riscv.esp.vst.h.64.ip(<8 x i8> %ev1, ptr %dst, i32 8)
  ret void
}

define dso_local void @test_vld_l_64_ip(ptr noundef %src, ptr noundef %dst) local_unnamed_addr #0 {
; ASM-LABEL: test_vld_l_64_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vld.l.64.ip q0, a0, 8
; ASM-NEXT:    esp.vst.l.64.ip q0, a1, 8
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <8 x i8>, ptr } @llvm.riscv.esp.vld.l.64.ip(ptr %src, i32 8)
  %ev1 = extractvalue { <8 x i8>, ptr } %v1, 0
  %v2 = tail call ptr @llvm.riscv.esp.vst.l.64.ip(<8 x i8> %ev1, ptr %dst, i32 8)
  ret void
}

define dso_local void @test_vld_64_xp(ptr noundef %src, ptr noundef %dst, i32 noundef %incr) local_unnamed_addr #2 {
; ASM-LABEL: test_vld_64_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vld.h.64.xp q0, a0, a2
; ASM-NEXT:    esp.vld.l.64.xp q1, a0, a2
; ASM-NEXT:    mv a0, a1
; ASM-NEXT:    esp.vst.h.64.xp q0, a0, a2
; ASM-NEXT:    esp.vst.l.64.xp q1, a1, a2
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <8 x i8>, ptr } @llvm.riscv.esp.vld.h.64.xp(ptr %src, i32 %incr)
  %ev1 = extractvalue { <8 x i8>, ptr } %v1, 0
  %ev2 = extractvalue { <8 x i8>, ptr } %v1, 1
  %v2 = tail call { <8 x i8>, ptr } @llvm.riscv.esp.vld.l.64.xp(ptr %ev2, i32 %incr)
  %ev3 = extractvalue { <8 x i8>, ptr } %v2, 0
  %v3 = tail call ptr @llvm.riscv.esp.vst.h.64.xp(<8 x i8> %ev1, ptr %dst, i32 %incr)
  %v4 = tail call ptr @llvm.riscv.esp.vst.l.64.xp(<8 x i8> %ev3, ptr %dst, i32 %incr)
  ret void
}

declare ptr @llvm.riscv.esp.vst.h.64.xp(<8 x i8>, ptr, i32) #3

declare ptr @llvm.riscv.esp.vst.l.64.xp(<8 x i8>, ptr, i32) #3

define dso_local void @test_vldbc_8_ip(ptr noundef %src, ptr noundef %dst) local_unnamed_addr #0 {
; ASM-LABEL: test_vldbc_8_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldbc.8.ip q0, a0, 1
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <16 x i8>, ptr } @llvm.riscv.esp.vldbc.8.ip.m(ptr %src, i32 1)
  %ev1 = extractvalue { <16 x i8>, ptr } %v1, 0
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %ev1, ptr %dst, i32 16)
  ret void
}

define dso_local void @test_vldbc_16_ip(ptr noundef %src, ptr noundef %dst) local_unnamed_addr #0 {
; ASM-LABEL: test_vldbc_16_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldbc.16.ip q0, a0, 2
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <8 x i16>, ptr } @llvm.riscv.esp.vldbc.16.ip.m(ptr %src, i32 2)
  %ev1 = extractvalue { <8 x i16>, ptr } %v1, 0
  %bc1 = bitcast <8 x i16> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst, i32 16)
  ret void
}

define dso_local void @test_vldbc_32_ip(ptr noundef %src, ptr noundef %dst) local_unnamed_addr #0 {
; ASM-LABEL: test_vldbc_32_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldbc.32.ip q0, a0, 4
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <4 x i32>, ptr } @llvm.riscv.esp.vldbc.32.ip.m(ptr %src, i32 4)
  %ev1 = extractvalue { <4 x i32>, ptr } %v1, 0
  %bc1 = bitcast <4 x i32> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst, i32 16)
  ret void
}

define dso_local void @test_vldbc_8_xp(ptr noundef %src, ptr noundef %dst, i32 noundef %incr) local_unnamed_addr #0 {
; ASM-LABEL: test_vldbc_8_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldbc.8.xp q0, a0, a2
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <16 x i8>, ptr } @llvm.riscv.esp.vldbc.8.xp.m(ptr %src, i32 %incr)
  %ev1 = extractvalue { <16 x i8>, ptr } %v1, 0
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %ev1, ptr %dst, i32 16)
  ret void
}

define dso_local void @test_vldbc_16_xp(ptr noundef %src, ptr noundef %dst, i32 noundef %incr) local_unnamed_addr #0 {
; ASM-LABEL: test_vldbc_16_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldbc.16.xp q0, a0, a2
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <8 x i16>, ptr } @llvm.riscv.esp.vldbc.16.xp.m(ptr %src, i32 %incr)
  %ev1 = extractvalue { <8 x i16>, ptr } %v1, 0
  %bc1 = bitcast <8 x i16> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst, i32 16)
  ret void
}

define dso_local void @test_vldbc_32_xp(ptr noundef %src, ptr noundef %dst, i32 noundef %incr) local_unnamed_addr #0 {
; ASM-LABEL: test_vldbc_32_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldbc.32.xp q0, a0, a2
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <4 x i32>, ptr } @llvm.riscv.esp.vldbc.32.xp.m(ptr %src, i32 %incr)
  %ev1 = extractvalue { <4 x i32>, ptr } %v1, 0
  %bc1 = bitcast <4 x i32> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst, i32 16)
  ret void
}

define dso_local void @test_vldext_s8_ip(ptr noundef %src, ptr noundef %dst1, ptr noundef %dst2) local_unnamed_addr #0 {
; ASM-LABEL: test_vldext_s8_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldext.s8.ip q0, q1, a0, 16
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    esp.vst.128.ip q1, a2, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <8 x i16>, <8 x i16>, ptr } @llvm.riscv.esp.vldext.s8.ip.m(ptr %src, i32 16)
  %ev1 = extractvalue { <8 x i16>, <8 x i16>, ptr } %v1, 0
  %ev2 = extractvalue { <8 x i16>, <8 x i16>, ptr } %v1, 1
  %bc1 = bitcast <8 x i16> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst1, i32 16)
  %bc2 = bitcast <8 x i16> %ev2 to <16 x i8>
  %vst_ptr.1 = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc2, ptr %dst2, i32 16)
  ret void
}

define dso_local void @test_vldext_s16_ip(ptr noundef %src, ptr noundef %dst1, ptr noundef %dst2) local_unnamed_addr #0 {
; ASM-LABEL: test_vldext_s16_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldext.s16.ip q0, q1, a0, 16
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    esp.vst.128.ip q1, a2, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <4 x i32>, <4 x i32>, ptr } @llvm.riscv.esp.vldext.s16.ip.m(ptr %src, i32 16)
  %ev1 = extractvalue { <4 x i32>, <4 x i32>, ptr } %v1, 0
  %ev2 = extractvalue { <4 x i32>, <4 x i32>, ptr } %v1, 1
  %bc1 = bitcast <4 x i32> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst1, i32 16)
  %bc2 = bitcast <4 x i32> %ev2 to <16 x i8>
  %vst_ptr.1 = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc2, ptr %dst2, i32 16)
  ret void
}

define dso_local void @test_vldext_u16_ip(ptr noundef %src, ptr noundef %dst1, ptr noundef %dst2) local_unnamed_addr #0 {
; ASM-LABEL: test_vldext_u16_ip:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldext.u16.ip q0, q1, a0, 16
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    esp.vst.128.ip q1, a2, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <4 x i32>, <4 x i32>, ptr } @llvm.riscv.esp.vldext.u16.ip.m(ptr %src, i32 16)
  %ev1 = extractvalue { <4 x i32>, <4 x i32>, ptr } %v1, 0
  %ev2 = extractvalue { <4 x i32>, <4 x i32>, ptr } %v1, 1
  %bc1 = bitcast <4 x i32> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst1, i32 16)
  %bc2 = bitcast <4 x i32> %ev2 to <16 x i8>
  %vst_ptr.1 = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc2, ptr %dst2, i32 16)
  ret void
}

define dso_local void @test_vldext_s8_xp(ptr noundef %src, ptr noundef %dst1, ptr noundef %dst2, i32 noundef %incr) local_unnamed_addr #0 {
; ASM-LABEL: test_vldext_s8_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldext.s8.xp q0, q1, a0, a3
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    esp.vst.128.ip q1, a2, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <8 x i16>, <8 x i16>, ptr } @llvm.riscv.esp.vldext.s8.xp.m(ptr %src, i32 %incr)
  %ev1 = extractvalue { <8 x i16>, <8 x i16>, ptr } %v1, 0
  %ev2 = extractvalue { <8 x i16>, <8 x i16>, ptr } %v1, 1
  %bc1 = bitcast <8 x i16> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst1, i32 16)
  %bc2 = bitcast <8 x i16> %ev2 to <16 x i8>
  %vst_ptr.1 = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc2, ptr %dst2, i32 16)
  ret void
}

define dso_local void @test_vldext_s16_xp(ptr noundef %src, ptr noundef %dst1, ptr noundef %dst2, i32 noundef %incr) local_unnamed_addr #0 {
; ASM-LABEL: test_vldext_s16_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldext.s16.xp q0, q1, a0, a3
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    esp.vst.128.ip q1, a2, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <4 x i32>, <4 x i32>, ptr } @llvm.riscv.esp.vldext.s16.xp.m(ptr %src, i32 %incr)
  %ev1 = extractvalue { <4 x i32>, <4 x i32>, ptr } %v1, 0
  %ev2 = extractvalue { <4 x i32>, <4 x i32>, ptr } %v1, 1
  %bc1 = bitcast <4 x i32> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst1, i32 16)
  %bc2 = bitcast <4 x i32> %ev2 to <16 x i8>
  %vst_ptr.1 = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc2, ptr %dst2, i32 16)
  ret void
}

define dso_local void @test_vldext_u8_xp(ptr noundef %src, ptr noundef %dst1, ptr noundef %dst2, i32 noundef %incr) local_unnamed_addr #0 {
; ASM-LABEL: test_vldext_u8_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldext.u8.xp q0, q1, a0, a3
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    esp.vst.128.ip q1, a2, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <8 x i16>, <8 x i16>, ptr } @llvm.riscv.esp.vldext.u8.xp.m(ptr %src, i32 %incr)
  %ev1 = extractvalue { <8 x i16>, <8 x i16>, ptr } %v1, 0
  %ev2 = extractvalue { <8 x i16>, <8 x i16>, ptr } %v1, 1
  %bc1 = bitcast <8 x i16> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst1, i32 16)
  %bc2 = bitcast <8 x i16> %ev2 to <16 x i8>
  %vst_ptr.1 = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc2, ptr %dst2, i32 16)
  ret void
}

define dso_local void @test_vldext_u16_xp(ptr noundef %src, ptr noundef %dst1, ptr noundef %dst2, i32 noundef %incr) local_unnamed_addr #0 {
; ASM-LABEL: test_vldext_u16_xp:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    esp.vldext.u16.xp q0, q1, a0, a3
; ASM-NEXT:    esp.vst.128.ip q0, a1, 16
; ASM-NEXT:    esp.vst.128.ip q1, a2, 16
; ASM-NEXT:    ret
entry:
  %v1 = tail call { <4 x i32>, <4 x i32>, ptr } @llvm.riscv.esp.vldext.u16.xp.m(ptr %src, i32 %incr)
  %ev1 = extractvalue { <4 x i32>, <4 x i32>, ptr } %v1, 0
  %ev2 = extractvalue { <4 x i32>, <4 x i32>, ptr } %v1, 1
  %bc1 = bitcast <4 x i32> %ev1 to <16 x i8>
  %vst_ptr = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc1, ptr %dst1, i32 16)
  %bc2 = bitcast <4 x i32> %ev2 to <16 x i8>
  %vst_ptr.1 = tail call ptr @llvm.riscv.esp.vst.128.ip(<16 x i8> %bc2, ptr %dst2, i32 16)
  ret void
}

declare { <16 x i8>, ptr } @llvm.riscv.esp.vld.128.ip(ptr, i32) #4

declare ptr @llvm.riscv.esp.vst.128.ip(<16 x i8>, ptr, i32) #3

declare { <16 x i8>, ptr } @llvm.riscv.esp.vld.128.xp(ptr, i32) #4

declare ptr @llvm.riscv.esp.vst.128.xp(<16 x i8>, ptr, i32) #3

declare { <16 x i8>, ptr, i32 } @llvm.riscv.esp.ld.128.usar.ip(ptr, i32) #4

declare { <16 x i8>, ptr, i32 } @llvm.riscv.esp.ld.128.usar.xp(ptr, i32) #4

declare { <8 x i8>, ptr } @llvm.riscv.esp.vld.h.64.ip(ptr, i32) #4

declare ptr @llvm.riscv.esp.vst.h.64.ip(<8 x i8>, ptr, i32) #3

declare { <8 x i8>, ptr } @llvm.riscv.esp.vld.l.64.ip(ptr, i32) #4

declare ptr @llvm.riscv.esp.vst.l.64.ip(<8 x i8>, ptr, i32) #3

declare { <8 x i8>, ptr } @llvm.riscv.esp.vld.h.64.xp(ptr, i32) #4

declare { <8 x i8>, ptr } @llvm.riscv.esp.vld.l.64.xp(ptr, i32) #4

declare { <16 x i8>, ptr } @llvm.riscv.esp.vldbc.8.ip.m(ptr, i32) #4

declare { <8 x i16>, ptr } @llvm.riscv.esp.vldbc.16.ip.m(ptr, i32) #4

declare { <4 x i32>, ptr } @llvm.riscv.esp.vldbc.32.ip.m(ptr, i32) #4

declare { <16 x i8>, ptr } @llvm.riscv.esp.vldbc.8.xp.m(ptr, i32) #4

declare { <8 x i16>, ptr } @llvm.riscv.esp.vldbc.16.xp.m(ptr, i32) #4

declare { <4 x i32>, ptr } @llvm.riscv.esp.vldbc.32.xp.m(ptr, i32) #4

declare { <8 x i16>, <8 x i16>, ptr } @llvm.riscv.esp.vldext.s8.ip.m(ptr, i32) #4

declare { <4 x i32>, <4 x i32>, ptr } @llvm.riscv.esp.vldext.s16.ip.m(ptr, i32) #4

declare { <4 x i32>, <4 x i32>, ptr } @llvm.riscv.esp.vldext.u16.ip.m(ptr, i32) #4

declare { <8 x i16>, <8 x i16>, ptr } @llvm.riscv.esp.vldext.s8.xp.m(ptr, i32) #4

declare { <4 x i32>, <4 x i32>, ptr } @llvm.riscv.esp.vldext.s16.xp.m(ptr, i32) #4

declare { <8 x i16>, <8 x i16>, ptr } @llvm.riscv.esp.vldext.u8.xp.m(ptr, i32) #4

declare { <4 x i32>, <4 x i32>, ptr } @llvm.riscv.esp.vldext.u16.xp.m(ptr, i32) #4

attributes #0 = { nounwind }
attributes #1 = { nounwind }
attributes #2 = { nounwind }
attributes #3 = { nounwind }
attributes #4 = { nounwind }

